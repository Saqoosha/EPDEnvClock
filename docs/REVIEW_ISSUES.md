...
**問題**: `setup()`のたびに`SensorManager_ReadBlocking(40000)`を呼んでおり、ウェイク後も最大40秒（平均でも十数秒）I2C待ちでボードが起きっぱなしになる。

**影響**:

- 1分周期で更新する設計なのに、実際は「58秒睡眠＋最大40秒待ち」で1サイクル≒1.5分
- 深睡眠による省電力がほぼ得られない
...
- `wakeFromSleep`がtrueなら非ブロッキング読み(`SensorManager_Read`)に切り替える
- RTCフラグで初回だけ待つよう分岐
...
### 1.3 フレームバッファ保存が常に失敗する圧縮バッファサイズ
...
- 最低でも`malloc(size * 2)`（もしくは`PSRAM`に`2*size`を用意）
- 圧縮成功率を100%にする
