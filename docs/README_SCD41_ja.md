...
センサー値は現在時刻・日付とともに電子ペーパーディスプレイに自動的に表示されます。システムは以下のように動作します：

1. ディープスリープからの復帰時にセンサーを初期化
2. シングルショット測定を実行（約5秒）
3. CO2（ppm）、温度（°C）、湿度（%RH）をEPDに表示
4. 次の分までディープスリープに入る

### シリアルモニターでのデバッグ

シリアルモニターを115200bpsで開くと、センサーの初期化状態やエラーメッセージを確認できます。
