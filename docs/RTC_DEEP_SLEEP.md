...
ESP32 の内蔵 150kHz RC オシレーターは温度依存で **おおむね 50〜450 ms/分** ドリフトする。
（初期観測では ~170 ms/分 付近だったが、低温や環境によって大きく変動する）
補正なしでは、1時間後に数秒〜十数秒のズレになる。
...
│    driftRate = α × old + (1-α) × trueRate (EMA, α=0.7*)      │
│    *NTPが長引き (>400ms) かつ残差が小さい (<300ms) 場合のみ α=0.5│
│      残差が大きいときは αを下げずに収束を優先                     │
└─────────────────────────────────────────────────────────────┘
```
...
| 項目 | 補正前 | 補正後 |
|------|--------|--------|
| 1分あたりドリフト | ~50-450 ms (温度依存) | EMA + クランプで収束 |
| 1時間あたりドリフト | 数秒〜十数秒 | サブ秒〜数秒レベル |
| 累積誤差 | 線形増加 | 自己修正 |

### ドリフトレート永続化

ドリフトレートはSDカード (`/drift_rate.txt`) に保存され、電源オフ後も維持される：

- NTP同期でレート更新時に自動保存
- 起動時にRTCメモリに較正値がなければSDカードから復元
- 初回起動のみデフォルト値 (50 ms/min) を使用

クランプ範囲: -600〜+600 ms/min（温度依存で実測値は大きく変動）
- 大きな残差かつ符号反転時は、即座に新レートを採用して立ち上がりを速くする
- NTP同期時間が長いときでも、残差が大きければ α を下げない（収束優先）
...
- 完了検出が確実
- ミリ秒精度の時刻取得
- NTPの 4 つのタイムスタンプ (t1/t2/t3/t4) を使ってオフセットと RTT を計算し、ネットワーク遅延バイアスを除去
...
trueDrift = residual + cumulativeCompensation  // 残差に累積補正を足して真のドリフトを推定
```
...
- RTC ドリフト: 約 50-450 ms/分（温度・個体差で変動）
- ESP32 内蔵 150kHz RC オシレーターの典型的な範囲
- 補正後の残差: サブ秒〜数秒/同期（クランプと EMA で段階的に収束）
- EMA（指数移動平均）で自動較正。NTPが長引いた場合でも、残差が大きいときは α を下げずに収束を優先
